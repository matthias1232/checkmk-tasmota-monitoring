...
def parse_tasmota_status(string_table: StringTable) -> Mapping[str, Any]:
    """Parse JSON aus <<<tasmota_status:sep(0)>>>."""
    if not string_table:
        return {}
    try:
        json_str = " ".join(row[0] for row in string_table if row)
...
agent_section_tasmota_status = AgentSection(
    name="tasmota_status",
    parse_function=parse_tasmota_status,
)